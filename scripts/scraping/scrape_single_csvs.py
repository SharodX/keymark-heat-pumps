...
def safe_filename(name: str) -> str:
    ...
    name = re.sub(r'[<>:"/\\|?*\t\r\n]', '_', name)
    # also strip leading/trailing dots and spaces (Windows does not allow them)
    name = name.strip(" .")
    return name
...
def import_keymark_data(i=0):
    ...
    foldername = 'csv/'
    input_dir = os.path.join(THIS_FOLDER_PATH, '..', 'input', foldername)
    ...
    for manufacturer_info in manufacturers_info.find_all('td')[i:]:
        i += 1
        print('Progress: ', i, ' / ', len(manufacturers_info.find_all('td')),
              ' In case of error: restart function with ', i-1,
              ' as input of function')
        manufacturer = manufacturer_info.text
        models_info = BeautifulSoup(
            requests.get('https://www.heatpumpkeymark.com/' +
                         manufacturer_info.a.get('href')).content,
            'html.parser'
        )
        # look for models:
        for model in models_info.find_all('td'):
            if model.text != manufacturer:
                filename = manufacturer + model.text
